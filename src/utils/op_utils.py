...
def shape_trick(tri_label, descriptor, edge_temp):
    metric_list = []
    for edge in edge_temp:
        if tri_label[1] in ["bigger than", "smaller than"]:
            metric_sub = descriptor[0][edge[0]][9]
            metric_obj = descriptor[1][edge[1]][9]
        if tri_label[1] in ["higher than", "lower than"]:
            metric_sub = descriptor[0][edge[0]][10]
            metric_obj = descriptor[1][edge[1]][10]

        metric_list.append(metric_sub - metric_obj)
        
    metric_list = torch.hstack(metric_list)
    if tri_label[1] in ["smaller than", "lower than"]:
        metric_list = -metric_list
        
    metric_list = 2 * (metric_list - torch.min(metric_list)) / (torch.max(metric_list) - torch.min(metric_list)) - 1
    if tri_label[1] == "same as":
        metric_list = torch.cos(metric_list)

    return metric_list * 10


def obj_prompt(obj_text):
    return np.array([f"a photo of a {i}" for i in obj_text])


def tri_prompt(tri):
    if tri[1] in ["left", "right"]:
        return "there is a %s on the %s of %s" % (tri[0], tri[1], tri[2])
    elif tri[1] == "front":
        return "there is a %s in %s of %s" % (tri[0], tri[1], tri[2])
    else:
        return "there is a %s %s %s" % (tri[0], tri[1], tri[2])


def generate_instruct(sub, obj, rel):
    if rel in ["left", "right"]:
        instruct = f"the probability that the {sub} on the {rel} of the {obj} in this picture"
    elif rel == "front":
        instruct = f"the probability that the {sub} in {rel} of the {obj} in this picture"
    else:
        instruct = f"the probability that the {sub} is {rel} the {obj} in this picture"
    return instruct
...
class Gen_edge_descriptor(MessagePassing):#TODO: move to model
    ...
    def forward(self, descriptor, edges_indices):
        ...
        coll_dict = self._collect(self._user_args, edges_indices, size, {"x":descriptor})  # 收集相关参数，将用户参数和父类需要的参数进行对应
        msg_kwargs = self.inspector.collect_param_data('message', coll_dict)  # 获得message函数的输入参数，也就是x_i, x_j
        edge_feature = self.message(**msg_kwargs)  # 消息传递   
        return edge_feature
