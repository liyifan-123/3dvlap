...
def evaluate_triplet_recallk(objs_pred, rels_pred, gt_rel, edges, multi_rel_outputs, topk, topk_each, use_clip=False, evaluate='triplet'):
    # objs_pred: N_o * 160
    # rels_pred: N_r * 26
    # gt_rel: N_r * 26, [sub_instance_id, obj_instance_id, predicate_list]
    # edges: N_r * 2, 0 - N_o-1 (obj index)
    # confidence_threshold: no
    # topk_each: 表示需要的最大的topk
    # topk: [20, 50, 100]
    res, triplet = [], []
    if not use_clip:
    ...
    for edge in range(len(edges)):
        ...
        rel_predictions = rels_pred[edge] # [26]
        sub = objs_pred[edge_from]  # [160]
        obj = objs_pred[edge_to]  # [160]  预测概率

        node_score = torch.einsum('n,m->nm',sub,obj)
        if evaluate == 'triplet':
            conf_matrix = torch.einsum('nl,m->nlm',node_score,rel_predictions)  # [160, 160, 26]
            conf_matrix_1d = conf_matrix.reshape(-1)  # [160*160*26]
        elif evaluate == 'rels':
        ...
        curr_topk_id = torch.stack([curr_edge_id, curr_topk_conf_id], dim=-1)  # [[边id,topk的triplet_id],.....]
        ...
        all_topk_id = all_topk_id[select_id]  # 记录整个场景的三元组topk的预测标签
    ...
    all_number = sum([min(1,len(gt_edge[2])) for gt_edge in gt_rel])  # 所有正确的三元组数量
    ...
    for idk, [edge, idx_1d] in enumerate(all_topk_id):  # calculate for each predicted edge
        ...
        edge = int(edge)
        if evaluate == 'triplet':
            idx = np.unravel_index(idx_1d, (size_o, size_o, size_r))  # 求出1维索引在原来的维度的索引值
