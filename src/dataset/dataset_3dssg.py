...
def dataset_loading_rio27(root:str, pth_selection:str,split:str,class_choice:list=None):
    ...
    if split == 'train_scans' :
        ...
        with open(os.path.join(root, 'train_rio27_no_split.json'), "r") as read_file:
        ...
        with open(os.path.join(root, 'validation_rio27_no_split.json'), "r") as read_file:
            data = json.load(read_file)
    else:
...
def dataset_loading_3RScan(root:str, pth_selection:str,split:str,class_choice:list=None):  
    ...
    if split == 'train_scans' :
        ...
        with open(os.path.join(root, 'train_no_split.json'), "r") as read_file:
            data = json.load(read_file)
    elif split == 'validation_scans':
        selected_scans = selected_scans.union(util.read_txt_to_list(os.path.join(pth_selection,'validation_scans.txt')))
        with open(os.path.join(root, 'validation_no_split.json'), "r") as read_file:
            data = json.load(read_file)
    else:
        raise RuntimeError('unknown split type:',split)
    return  classNames, relationNames, data, selected_scans
...
class SSGDatasetGraph(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        obj_points, obj_2d_feats, rel_points, gt_rels, gt_class, edge_indices, descriptor, origin_obj_points = \
            self.data_preparation(points, instances, self.mconfig.num_points, self.mconfig.num_points_union,
                         ...
                         multi_view_root=self.config.multi_view_root)  # self.config.multi_view_root
        ...
        while(len(rel_points) == 0 or gt_rels.sum()==0) and self.for_train:  # 如果当前的index无法正常获取到relation points，就再运行一遍
            ...
            if self.use_descriptor:
                if self.use_2d_feats:
                    obj_points, obj_2d_feats, rel_points, gt_class, gt_rels, edge_indices, descriptor, scan_id_no_split, scan_split_idx, origin_obj_points = self.__getitem__(index)
                else:
                    obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, scan_id_no_split, scan_split_idx, origin_obj_points = self.__getitem__(index)
            else:
                obj_points, rel_points, gt_class, gt_rels, edge_indices, scan_id_no_split, scan_split_idx, origin_obj_points = self.__getitem__(index)

        if self.use_descriptor:
            if self.use_2d_feats:
                return obj_points, obj_2d_feats, rel_points, gt_class, gt_rels, edge_indices, descriptor, scan_id_no_split, scan_split_idx, origin_obj_points
            return obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, scan_id_no_split, scan_split_idx, origin_obj_points

        return obj_points, rel_points, gt_class, gt_rels, edge_indices, scan_id_no_split, scan_split_idx, origin_obj_points
    ...
    def data_preparation(self, points, instances, num_points, num_points_union, scene_id="",
        ...
        for i, instance_id in enumerate(nodes):
            ...
            if multi_view_root is not None:
                obj_2d_feats[i] = np.load(os.path.join(multi_view_root, f'data/3RScan/{scene_id}/multi_view_for_vlsat_main/instance_{instance_id}_origin_view_mean.npy'))
        ...
        return obj_points, obj_2d_feats, rel_points, gt_rels, label_node, edge_indices, descriptor, origin_obj_points
