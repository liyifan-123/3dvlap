...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, way='mul', use_knn=False):
        ...
        q = q.view(b_s, nq, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, head, nq, d_k)
        k = self.fc_k(keys).view(b_s, nk, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, head, d_k, nk)
        v = self.fc_v(values).view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, head, nk, d_v)

        att = torch.matmul(q, k) / np.sqrt(self.d_k)  # (b_s, head, nq, nk)
        att_map = att.clone()
        if use_knn:
        ...
        if attention_mask is not None:
            att = att.masked_fill(attention_mask==0, -np.inf)  # 让不同场景的物体之间的attention为负无穷
