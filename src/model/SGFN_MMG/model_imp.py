...
class IMP(BaseModel):
    ...
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        self.gcn = TripletIMP(512, num_layers=2)
    ...
    def process_val(self, result_print, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, 
                    edge_indices, batch_ids, scan_id, split_id, origin_obj_points, with_log=False, use_triplet=False, cal_recall=False):
 
        obj_pred, rel_pred = self(obj_points, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=False)
        ...
        if use_triplet:
            ...
            if cal_recall:
                triplet_recall_list_wo_gc = evaluate_triplet_recallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, [20, 50, 100], 1000, use_clip=False, evaluate='triplet')
                relation_recall_list_wo_gc = evaluate_triplet_recallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, [20, 50, 100], 1000, use_clip=False, evaluate='rels')
                
                triplet_recall_list_w_gc = evaluate_triplet_recallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, [20, 50, 100], 1, use_clip=False, evaluate='triplet')
                relation_recall_list_w_gc = evaluate_triplet_recallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, [20, 50, 100], 1, use_clip=False, evaluate='rels')

                triplet_mean_recall_list_w_gc = evaluate_triplet_mrecallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, [20, 50, 100], 1, use_clip=False, evaluate='triplet')
                relation_mean_recall_list_w_gc = evaluate_triplet_mrecallk(obj_pred.detach().cpu(), rel_pred.detach().cpu(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, [20, 50, 100], 1, use_clip=False, evaluate='rels')
        else:
            top_k_triplet = [101]
            cls_matrix = None
            sub_scores = None
