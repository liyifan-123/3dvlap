...
from src.model.model_utils.network_MMG import MMG_vlsat
...
from process_data.seesawLoss import SeesawLossWithLogits, binary_cross_entroy_loss
from process_data.seesaw_loss_new import SeesawLoss
...
class Mmgnet(BaseModel):
    def __init__(self, config, classNames, relationNames, dim_descriptor=11):
        ...
        self.num_rel=len(self.relationNames)
        ...
        self.init_weight(adapter_path=mconfig.adapter_path)
        ...
        # self.weight_dict, self.idx_obj_dict = create_scene_data_weight_binary()
        # self.rel_2d_loss = SeesawLossWithLogits()
        # self.rel_3d_loss = SeesawLossWithLogits()

    def init_weight(self, adapter_path):
        ...
        obj_text_features, rel_text_feature = self.get_label_weight()
    ...
    def get_rel_emb(self, objs_target, rels_target, edges):
        ...
        for edge_index in range(len(edges)):
            ...
            target_eo = self.classNames[objs_target[idx_eo]]  # 获取subject的物体名称
            target_os = self.classNames[objs_target[idx_os]]  # 获取object的物体名称
            ...
            if rels_target[edge_index].sum() == 0:
                ...
                for i in range(rels_target.shape[-1]):  # 这里用for循环是因为两个物体之间的relation标签可能不止一种
                    if rels_target[edge_index][i] == 1:
                        target_rel = self.relationNames[i]
        ...
        prompt_features = torch.cat(target_rel_tokens).cuda(self.config.DEVICE)
        with torch.no_grad():
            triplet_feats = self.clip_model.encode_text(prompt_features)  # 将text编码成特征
        
        # deal with multi label
        for edge_index in range(len(edges)):
            multi_rel_idxs = torch.where(torch.tensor(rel_index) == edge_index)[0]
            target_rel_feats.append(triplet_feats[multi_rel_idxs].reshape(len(multi_rel_idxs), 512).mean(0))  # 有两个物体之间可能存在多个relation，但是text经过clip编码的特征一个relation只能有一个，所以这里取平均
    ...
    def generate_object_pair_features(self, obj_feats, edges_feats, edge_indice):
        """
        object_pair_features的生成通过将物体特征和边特征进行拼接再过MLP构造。
        """
        obj_pair_feats = []
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        if istrain:
            obj_feature_3d_mimic = obj_feature[..., :512].clone()  # [B * num_obj_per_scan, 512]
        ...
        ''' Create edge feature '''
        with torch.no_grad():
            # 利用MessagePassing来实现edge初始特征的构建，edge特征主要通过descriptor的特征获得
            edge_feature = op_utils.Gen_edge_descriptor(flow=self.flow)(descriptor, edge_indices)  # [72, 11, 1] 72表示全连接图边的数量，11是特征维度，1是表示一个节点

        rel_feature_2d = self.rel_encoder_2d(edge_feature)  # pointnet 但是没有stn3k的旋转矩阵，因为相当于只有一个点输入
        rel_feature_3d = self.rel_encoder_3d(edge_feature)  # pointnet 但是没有stn3k的旋转矩阵，因为相当于只有一个点输入
        ...
        # gcn_obj_feature_3d, gcn_obj_feature_2d, gcn_edge_feature_3d, gcn_edge_feature_2d = obj_feature, obj_2d_feats, rel_feature_3d, rel_feature_2d
        ...
        gcn_edge_feature_2d_dis = self.triplet_projector_2d(gcn_edge_feature_2d_dis)  # MLP [edge_num, 512]

        rel_cls_3d = self.rel_predictor_3d(gcn_edge_feature_3d)  # PointNet的分类函数
    ...
    def process_train(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None):
        self.iteration +=1    # 看来这个是用来记处理了多少个batch

        obj_logits_3d, obj_logits_2d, rel_cls_3d, rel_cls_2d, obj_feature_3d, obj_feature_2d, edge_feature_2d, obj_logit_scale = self(obj_points, obj_2d_feats, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=True)  # 这里self()的写法意思是调用forword函数
        ...
        if self.mconfig.multi_rel_outputs:
            if self.mconfig.WEIGHT_EDGE == 'BG':
                ...
                batch_mean = torch.cat([zeros,batch_mean],dim=0)  # 记录一个batch下每种关系类别的数量
            ...
            # own binary cross entropy loss
            # loss_rel_3d = binary_cross_entroy_loss(rel_cls_3d, gt_rel_cls)
            # loss_rel_2d = binary_cross_entroy_loss(rel_cls_2d, gt_rel_cls)

            # use new seesawloss
            # loss_rel_3d = self.rel_3d_loss(rel_cls_3d, gt_rel_cls)
            # loss_rel_2d = self.rel_2d_loss(rel_cls_2d, gt_rel_cls)
            
            # 使用真实的object label
            ####################################################
            # relation_idx_2d = relation_idx_3d = gt_cls[edge_indices].cpu().tolist()
            ####################################################

            # 使用预测的object label
            ####################################################
            # relation_idx_3d = obj_logits_3d.max(-1)[1][edge_indices].cpu().tolist()
            # relation_idx_2d = obj_logits_2d.max(-1)[1][edge_indices].cpu().tolist()
            ####################################################


            # loss_rel_3d = torch.zeros(len(relation_idx_3d)).cuda(self.config.DEVICE)
            # loss_rel_2d = torch.zeros(len(relation_idx_2d)).cuda(self.config.DEVICE)
            
            # for i in range(len(relation_idx_2d)):
            #     key_2d = tuple([self.idx_obj_dict[relation_idx_2d[i][0]], self.idx_obj_dict[relation_idx_2d[i][1]]])
            #     key_3d = tuple([self.idx_obj_dict[relation_idx_3d[i][0]], self.idx_obj_dict[relation_idx_3d[i][1]]])
            #     try:
            #         weight_2d = self.weight_dict[key_2d].cuda(self.config.DEVICE)
            #     except:
            #         weight_2d = torch.ones([26]).cuda(self.config.DEVICE)
            #     try:
            #         weight_3d = self.weight_dict[key_3d].cuda(self.config.DEVICE)
            #     except:
            #         weight_3d = torch.ones([26]).cuda(self.config.DEVICE)
            #     # loss_rel_3d[i] = self.rel_3d_loss(rel_cls_3d[i], gt_rel_cls[i], weight_3d)
            #     # loss_rel_2d[i] = self.rel_2d_loss(rel_cls_2d[i], gt_rel_cls[i], weight_2d)
            #     loss_rel_3d[i] = F.binary_cross_entropy(rel_cls_3d[i], gt_rel_cls[i], weight_3d)
            #     loss_rel_2d[i] = F.binary_cross_entropy(rel_cls_2d[i], gt_rel_cls[i], weight_2d)
            
            # loss_rel_3d = loss_rel_3d.mean()
            # loss_rel_2d = loss_rel_2d.mean()
        else:
        ...
        loss_mimic = self.cosine_loss(obj_feature_3d, obj_feature_2d, t=0.8)  # 在2D和3D的对应物体之间构造一个损失函数，让3D靠近2D的特征
    ...
    def process_val(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, scan_id=None, split_id=None, origin_obj_points=None, with_log=False, use_triplet=False):
        ...
        top_k_obj = evaluate_topk_object(obj_logits_3d.detach(), gt_cls, topk=11)  # 存每一个物体正确类别的预测概率在所有类别的预测概率的排序，最大不超过12
        gt_edges = get_gt(gt_cls, gt_rel_cls, edge_indices, self.mconfig.multi_rel_outputs)  # 从one-hot编码变为类别列表
        top_k_rel = evaluate_topk_predicate(rel_cls_3d.detach(), gt_edges, self.mconfig.multi_rel_outputs, topk=6)
        ...
        top_k_rel_2d = evaluate_topk_predicate(rel_cls_2d.detach(), gt_edges, self.mconfig.multi_rel_outputs, topk=6)
        ...
        if use_triplet:
            top_k_triplet, cls_matrix, sub_scores, obj_scores, rel_scores = evaluate_triplet_topk(obj_logits_3d.detach(), rel_cls_3d.detach(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=101, use_clip=True, obj_topk=top_k_obj)
            top_k_2d_triplet, _, _, _, _ = evaluate_triplet_topk(obj_logits_2d.detach(), rel_cls_2d.detach(), gt_edges, edge_indices, self.mconfig.multi_rel_outputs, topk=101, use_clip=True, obj_topk=top_k_obj)
        else:
            top_k_triplet = [101]
            cls_matrix = None
            sub_scores = None
