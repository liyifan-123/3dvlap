...
from src.model.model_utils.network_imp import TripletIMP
...
class Mmgnet(BaseModel):
    def __init__(self, config, classNames, relationNames, dim_descriptor=11):
        ...
        self.mmg = TripletIMP(512, num_layers=2)
        ...
        self.optimizer = optim.AdamW([
            ...
            {'params':mmg_obj, 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            ...
            # {'params':rnn_list, 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
            # {'params':self.obj_res_conv, 'lr':float(config.LR), 'weight_decay':self.config.W_DECAY, 'amsgrad':self.config.AMSGRAD},
        ])
    ...
    def get_rel_label(self, obj_label, tri_texts, batch_ids, obj_features, relation_fea = None, descriptor = None, img_pair_info=None, img_pair_idx=None):
        """Match and confirm the features of the relation using the features of the 
        triplet text and the three-dimensional features of the subject and object."""
    ...
    def get_rel_label_no_mask(self, tri_texts, batch_ids, obj_features, relation_fea = None, distance_weight = None, descriptor = None, img_pair_info=None, img_pair_idx=None):
        """Match and confirm the features of the relation using the 
        features of the triplet text and the three-dimensional features of the subject and object."""
        def change_array_to_dict(tri_texts):
            tri_dict = {}
            for key in tri_texts:
                k = tuple(key)
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor, batch_ids, obj_texts = None, tri_texts = None, img_pair_info=None, img_pair_idx=None, istrain=False):
        ...
        gcn_obj_feature_3d, gcn_edge_feature_3d = self.mmg(obj_feature, rel_feature_3d, edge_indices)
        ...
        if istrain:
            ...
            return obj_logits_3d, rel_cls_3d, object_label_backup
    ...
    def process_train(self, obj_points, obj_2d_feats, descriptor, edge_indices, batch_ids, obj_texts, tri_texts, gt_rel_cls, gt_class, rel_count_class = None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None, img_pair_info=None, img_pair_idx=None):
        ...
        obj_logits_3d, rel_cls_3d, obj_feature_3d, obj_feature_2d, obj_logit_scale, obj_persudo_label, rel_persudo_label, visual_rel_feature_3d = self(obj_points, obj_2d_feats, edge_indices.t().contiguous(), descriptor, batch_ids, obj_texts, tri_texts, img_pair_info=img_pair_info, img_pair_idx=img_pair_idx, istrain=True)  
    ...
    def process_val(self, result_print, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, scan_id=None, split_id=None, origin_obj_points=None, with_log=False, use_triplet=False, cal_recall=False):
        ...
        obj_logits_3d, rel_cls_3d, object_label_backup = self(obj_points, obj_2d_feats, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=False)
        
        # compute metric
        top_k_obj_3d = evaluate_topk_object(obj_logits_3d.detach(), gt_cls, topk=11)  

        top_k_obj_2d = evaluate_topk_object(obj_logits_3d.detach(), gt_cls, topk=11)  
