...
class SGPN(BaseModel):
    ...
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        self.gcn = GraphTripleConvNet(512, num_layers=3)
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        for i in range(len(split_batch_ids)):
            ...
            node_fea, edge_fea = self.gcn(obj_temp_fea, edge_temp_fea, edge_indices)
            gcn_obj_feature.append(node_fea)
            gcn_rel_feature.append(edge_fea)

        gcn_obj_feature = torch.vstack(gcn_obj_feature)
        gcn_rel_feature = torch.vstack(gcn_rel_feature)
    ...
    def process_val(self, result_print, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, 
                    edge_indices, batch_ids, scan_id, split_id, origin_obj_points, with_log=False, use_triplet=False):
