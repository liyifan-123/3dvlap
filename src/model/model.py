...
class MMGNet():
    def __init__(self, config):
        ...
        if config.MODE  == 'train':
            ...
            self.dataset_train = build_dataset(self.config,split_type='train_scans', shuffle_objs=True,
                                               ...
                                               use_normal=mconfig.USE_NORMAL, Type="origin")
    ...
    def validation(self, debug_mode = False):
        ...
        # result_print = Result_print(self.config.exp, self.dataset_valid.classNames, self.dataset_valid.relationNames)
        ...
        for i, items in enumerate(val_loader, 0):
            ...
            with torch.no_grad():
                ...
                top_k_obj, top_k_obj_2d, top_k_rel, top_k_rel_2d, tok_k_triplet, top_k_2d_triplet, cls_matrix, sub_scores, obj_scores, rel_scores \
                    = self.model.process_val(obj_points, obj_2d_feats, gt_class, descriptor, gt_rel_cls, edge_indices, batch_ids, scan_id, split_id, origin_obj_points, use_triplet=True)
            ...
            # topk_3d_recall_list.append(topk_3d_recall)
            ...
            logs = [("Acc@1/obj_cls_acc", (topk_obj_list <= 1).sum() * 100 / len(topk_obj_list)),  # 因为topk_*_list统计的是真实的class的预测概率排序位置，所以用sum的方法
        ...
        # self.result_print.print_result(self.config.exp)
        ...
        # topk_3d_recall_result = np.array(topk_3d_recall_list).mean(0)
        # print(f"Eval: 3d recall@20 : {topk_3d_recall_result[0]}", file=f_in)
        # print(f"Eval: 3d recall@50 : {topk_3d_recall_result[1]}", file=f_in)
        # print(f"Eval: 3d recall@100 : {topk_3d_recall_result[2]}", file=f_in)
