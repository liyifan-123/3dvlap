...
class GraphEdgeAttenNetworkLayers(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, dim_atten, num_layers, num_heads=1, aggr= 'max', 
                 use_bn=False,flow='target_to_source',attention = 'fat', use_edge:bool=True, device = None, **kwargs):
        super().__init__()
        self.device = device
    ...
    def forward(self, node_feature, edge_feature, edges_indices, obj_center, batch_ids):
        ...
        if obj_center is not None:
            ...
            mask = torch.zeros(1, 1, N_K, N_K).cuda(self.device)
            distance = torch.zeros(1, self.num_heads, N_K, N_K).cuda(self.device)
            count = 0
