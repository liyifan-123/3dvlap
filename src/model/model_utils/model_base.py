...
class BaseModel(nn.Module):
    ...
    def save(self):
        ...
        for name, para in self._parameters.items():
            torch.save(para, os.path.join(self.saving_pth, name + suffix))
    ...
    def load(self, best=False):
        ...
        # load logit scale param 
        # for name, para in self._parameters.items():
        #     # para = torch.load(os.path.join(self.saving_pth, name + suffix))
        #     try:
        #         self._parameters[name] = torch.load(os.path.join(self.saving_pth, name + suffix))
        #     except:
        #         print("%s does not exist." % (os.path.join(self.saving_pth, name + suffix)))
        
        if os.path.exists(os.path.join(self.saving_pth,'optimizer'+suffix)):
            data = torch.load(os.path.join(self.saving_pth,'optimizer'+suffix), map_location=self.device)
        ...
        if os.path.exists(os.path.join(self.saving_pth,'lr_scheduler'+suffix)):
            data = torch.load(os.path.join(self.saving_pth,'lr_scheduler'+suffix), map_location=self.device)
