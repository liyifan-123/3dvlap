...
def load_rgb(path, target_name = define.LABEL_FILE_NAME):
    ...
    if pth_label != pth_label_raw:
        ...
        if 'nx' not in label_mesh_align.metadata['_ply_raw']['vertex']['data']:
            label_mesh_align.metadata['_ply_raw']['vertex']['properties']['nx'] = '<f4'
            label_mesh_align.metadata['_ply_raw']['vertex']['properties']['ny'] = '<f4'
            label_mesh_align.metadata['_ply_raw']['vertex']['properties']['nz'] = '<f4'
            label_mesh_align.metadata['_ply_raw']['vertex']['data']['nx'] = np.zeros([len(query_points),1],dtype=np.float32)
            label_mesh_align.metadata['_ply_raw']['vertex']['data']['ny'] = np.zeros([len(query_points),1],dtype=np.float32)
            label_mesh_align.metadata['_ply_raw']['vertex']['data']['nz'] = np.zeros([len(query_points),1],dtype=np.float32)
        for i in range(len(query_points)):
            ...
            label_mesh_align.metadata['_ply_raw']['vertex']['data']['red'][i] = colors[idx[0]][0]
            label_mesh_align.metadata['_ply_raw']['vertex']['data']['green'][i] = colors[idx[0]][1]
            label_mesh_align.metadata['_ply_raw']['vertex']['data']['blue'][i] = colors[idx[0]][2]
            if hasattr(mesh, 'vertex_normals'):
                label_mesh_align.metadata['_ply_raw']['vertex']['data']['nx'][i] = mesh.vertex_normals[idx[0]][0]
                label_mesh_align.metadata['_ply_raw']['vertex']['data']['ny'][i] = mesh.vertex_normals[idx[0]][1]
                label_mesh_align.metadata['_ply_raw']['vertex']['data']['nz'][i] = mesh.vertex_normals[idx[0]][2]
    else:
        #trimesh.base.Trimesh
        label_mesh_align.visual.vertex_colors = mesh.visual.vertex_colors
        label_mesh_align.metadata['_ply_raw']['vertex']['data']['nx'] = mesh.vertex_normals[:,0]
        label_mesh_align.metadata['_ply_raw']['vertex']['data']['ny'] = mesh.vertex_normals[:,1]
        label_mesh_align.metadata['_ply_raw']['vertex']['data']['nz'] = mesh.vertex_normals[:,2]
        label_mesh_align.metadata['_ply_raw']['vertex']['data']['red']   = mesh.visual.vertex_colors[:,0]
        label_mesh_align.metadata['_ply_raw']['vertex']['data']['green'] = mesh.visual.vertex_colors[:,1]
        label_mesh_align.metadata['_ply_raw']['vertex']['data']['blue']  = mesh.visual.vertex_colors[:,2]
